// Solves the (English) peg solitaire board game.
...
// The board must be surrounded by 2 illegal fields in each direction so that
// move() doesn't need to check the board boundaries. Periods represent illegal
// fields, ● are pegs, and ○ are holes.
...
var center int  // position of center hole if there's only one, otherwise -1
...
// in direction dir. If the move is valid, it's executed and move returns true.
  ...
  for pos, field := range board {
    ...
    if field == '●' {
      ...
      for _, dir := range [...]int{-1, -N, +1, +N} {
        // try each direction
        if move(pos, dir) {
          // a valid move was found and executed, check for solution
          if solve() {
            unmove(pos, dir)
